...
class DashboardWriterTests(unittest.TestCase):
    ...
    def test_write_outputs_generates_dashboard_team_and_section_pages(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Organization Status", dashboard_html)
            self.assertIn("High-priority needs", dashboard_html)
            self.assertIn("MLB Leaders", dashboard_html)
            self.assertIn("Top MLB hitters", dashboard_html)
            self.assertIn("Pressure Points", dashboard_html)

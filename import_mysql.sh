...
usage() {
  cat <<'EOF'
Usage:
  ./import_mysql.sh <database_name> [mysql options...]

Examples:
  ./import_mysql.sh ootp
  ./import_mysql.sh ootp -u root -p
  ./import_mysql.sh ootp --host 127.0.0.1 --port 3306 -u root -p
...
Notes:
  ...
  - Foreign keys are applied at the very end from `mysql/foreign_keys.mysql.sql`.
EOF
}
...
if [[ ! -d "$MYSQL_DIR" ]]; then
  echo "Error: mysql directory not found at $MYSQL_DIR" >&2
  exit 1
fi
